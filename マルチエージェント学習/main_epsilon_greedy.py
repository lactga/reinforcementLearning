...
from TreasureHuntingEnvironment import TreasureHuntingEnvironment as Environment
from PolicyEpsilonGreedy import PolicyEpsilonGreedy
from AgentSimpleProfitSharing import AgentSimpleProfitSharing as Agent
...
class Main:
    def __init__(self, cycle_num, seed, epsilon, reward=10, p=0.9):
        ...
        self.agent = Agent(policy_cls=PolicyEpsilonGreedy(epsilon=epsilon), cbid=0.1)
    ...
    def main(self, is_print_loop_counter=True, is_print_Qtable=False, is_print_action=False):
        for i in range(self.cycle_num):
            if is_print_loop_counter:
                print(i)
            self.environment = Environment(p=self.p, reward=self.reward)
